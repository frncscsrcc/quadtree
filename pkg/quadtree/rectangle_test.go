...
func TestHelperMethods(t *testing.T) {
	r, _ := NewRectangle(1, 2, 3, 4)
	if r.X1() != 1 {
		t.Errorf("X1() retunred invalid value %d instead of 1", r.X1())
	}
	if r.Y1() != 2 {
		t.Errorf("Y1() retunred invalid value %d instead of 2", r.Y1())
	}
	if r.X2() != 3 {
		t.Errorf("X2() retunred invalid value %d instead of 3", r.X2())
	}
	if r.Y2() != 4 {
		t.Errorf("Y2() retunred invalid value %d instead of 4", r.Y2())
	}
}
